...
bool PDDLProblemGenerator::printDomainName(std::ofstream &pFile) {
...
bool PDDLProblemGenerator::printObjects(std::ofstream &pFile) {
    ...
    if (getTypesClient.call(typeSrv)) {
        ...
        for(size_t t=0; t<typeSrv.response.types.size(); t++) {
            ...
            if (!getInstancesClient.call(instanceSrv)) {
                ...
                if(instanceSrv.response.instances.size() == 0) {
                    continue;
                }
...
bool PDDLProblemGenerator::printPredicateParameters(DomainFormulas &domainFormulas,
...
bool PDDLProblemGenerator::printPropositions(ros::Time &time, std::ofstream &pFile) {
        ...
        for(; domainIterator != domainAttributes.end(); ++domainIterator) {
            ...
            if (!getPropsClient.call(attrSrv)) {
                ...
                for(size_t i = 0; i < attributes.size(); ++i) {
                    ...
                    if (!printPredicateParameters(domainAttributes, attributes[i], pFile)) {
                        success = false;
                        break;
                    }
...
bool PDDLProblemGenerator::printFunctions(ros::Time &time, std::ofstream &pFile) {
    ...
    if (getDomainFuncsClient.call(domainAttrSrv)) {
        ...
        for(; domainIterator != domainAttrSrv.response.items.end(); ++domainIterator) {
            ...
            if (!getFuncsClient.call(attrSrv)) {
                ROS_ERROR("KCL: (PDDLProblemGenerator) Failed to call service %s: %s", 
                            state_function_service.c_str(), attrSrv.request.predicate_name.c_str());
                success = false;
                break;
            } else {
                ...
                for(size_t i = 0; i < attributes.size(); ++i) {
                    ...
                    pFile << "    (";
                    if(time < attributes[i].initial_time) {
                        pFile << "at " << (attributes[i].initial_time - time).toSec() << " (";
                    }
                    pFile << "= (" << attributes[i].attribute_name;
                    ...
                    pFile << ") " << attributes[i].function_value << ")";
                    if(time < attributes[i].initial_time) {
                        pFile << ")";
                    }
...
bool PDDLProblemGenerator::makeHeader(std::ofstream &pFile) {
    ...
    bool success = true;        
    success = success && printDomainName(pFile);
    success = success && printObjects(pFile);        
...
bool PDDLProblemGenerator::makeInitialState(std::ofstream &pFile) {
    ...
    pFile << "(:init" << std::endl;
    success = success && printPropositions(time, pFile);
    success = success && printFunctions(time, pFile);        
    pFile << ")" << std::endl;
    ...
    if (getCurrentGoalsClient.call(currentGoalSrv) && getDomainPropsClient.call(domainAttrSrv)) {
        ...
        for(size_t i = 0;i < goalAttributes.size(); ++i) {                
            ...
            if(goalAttributes[i].knowledge_type == rosplan_knowledge_msgs::KnowledgeItem::FACT) {
                ...
                if (!printPredicateParameters(domainAttributes, goalAttributes[i], pFile)) {
                    success = false;
                    break;
                }
...
void PDDLProblemGenerator::makeProblem(std::ofstream &pFile) {
    ...
    success = success && makeMetric(pFile);
    makeFooter(pFile);
}
